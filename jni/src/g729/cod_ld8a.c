  ...
  *----------------------------------------------------------------------*/
...

  {
    ...
    for(i=1; i<=M; i++)    /* Ap1[i] = Ap[i] - 0.7 * Ap[i-1]; */
       Ap1[i] = sub_g729(Ap[i], mult(Ap[i-1], 22938));
  ...
  for (i_subfr = 0;  i_subfr < L_FRAME; i_subfr += L_SUBFR)
  {
    ...
    *-----------------------------------------------------------------*/
    ...
    for (i = 0; i < L_SUBFR; i++)
    {
      ...
      xn2[i] = sub_g729(xn[i], extract_h(L_temp));
    }
    ...
    *ana++ = Qua_gain_G729(code, g_coeff_cs, exp_g_coeff_cs,
    ...
    *------------------------------------------------------*/
    ...
    for (i = L_SUBFR-M, j = 0; i < L_SUBFR; i++, j++)
    {
      ...
      state->mem_w0[j]  = sub_g729(xn[i], add(temp, k));
    }
