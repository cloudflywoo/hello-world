...
)
{
 ...
 static Word16 past_qua_en[4] = { -14336, -14336, -14336, -14336 };
   ...
   *-----------------------------------------------------------------*/
   ...
   //if( sub_g729(exp1, exp2)>0 ){
   if( exp1 > exp2 ){
      L_tmp = L_sub( L_shr( L_tmp1, sub_g729(exp1,exp2) ), L_tmp2 );
      exp = exp2;
   }
   else{
      L_tmp = L_sub( L_tmp1, L_shr( L_tmp2, sub_g729(exp2,exp1) ) );
      exp = exp1;
   }
   sft = norm_l_g729( L_tmp );
   denom = extract_h( L_shl(L_tmp, sft) );
   exp_denom = sub_g729( add( exp, sft ), 16 );

   inv_denom = div_s_g729(16384,denom);
   inv_denom = negate( inv_denom );
   exp_inv_denom = sub_g729( 14+15, exp_denom );
   ...
   *-----------------------------------------------------------------*/
   ...
   //if( sub_g729(exp1, exp2)>0 ){
   if (exp1 > exp2){
      L_tmp = L_sub( L_shr( L_tmp1, add(sub_g729(exp1,exp2),1 )), L_shr( L_tmp2,1 ) );
      exp = sub_g729(exp2,1);
   }
   else{
      L_tmp = L_sub( L_shr( L_tmp1,1 ), L_shr( L_tmp2, add(sub_g729(exp2,exp1),1 )) );
      exp = sub_g729(exp1,1);
   }
   sft = norm_l_g729( L_tmp );
   nume = extract_h( L_shl(L_tmp, sft) );
   exp_nume = sub_g729( add( exp, sft ), 16 );

   sft = sub_g729( add( exp_nume, exp_inv_denom ), (9+16-1) );
   L_acc = L_shr( L_mult( nume,inv_denom ), sft );
   best_gain[0] = extract_h( L_acc );             /*-- best_gain[0]:Q9 --*/

   if (tameflag == 1){
     //if(sub_g729(best_gain[0], GPCLIP2) > 0) best_gain[0] = GPCLIP2;
   ...
   *-----------------------------------------------------------------*/
   ...
   sft = sub_g729( add( exp_nume, exp_inv_denom ), (2+16-1) );
   L_acc = L_shr( L_mult( nume,inv_denom ), sft );
   best_gain[1] = extract_h( L_acc );             /*-- best_gain[1]:Q2 --*/

   /*--- Change Q-format of gcode0 ( Q[exp_gcode0] -> Q4 ) ---*/
   //if( sub_g729(exp_gcode0,4) >= 0 ){
   if (exp_gcode0 >=4) {
      gcode0_org = shr_g729( gcode0, sub_g729(exp_gcode0,4) );
   }
   else{
      L_acc = L_deposit_l_g729( gcode0 );
      L_acc = L_shl( L_acc, sub_g729( (4+16), exp_gcode0 ) );
 ...
 *---------------------------------------------------------------------------*/
   ...
   exp_min[2] = add( exp_coeff[2], sub_g729( shl( exp_gcode0, 1 ), 21 ) );
   exp_min[3] = add( exp_coeff[3], sub_g729( exp_gcode0, 3 ) );
   exp_min[4] = add( exp_coeff[4], sub_g729( exp_gcode0, 4 ) );

   e_min = exp_min[0];
   for(i=1; i<5; i++){
      //if( sub_g729(exp_min[i], e_min) < 0 ){
     if (exp_min[i] < e_min) {
         e_min = exp_min[i];
      }
   }
  ...
  *--------------------------------------------------------------------------*/
   ...
   if(gcode0>0){
      ...
      *cand1 = 0 ;
      ...
      //} while(sub_g729((*cand1),(NCODE1-NCAN1))<0) ;
      ...
      *cand2 = 0 ;
      ...
      //} while(sub_g729((*cand2),(NCODE2-NCAN2))<0) ;
      ...
      *cand1 = 0 ;
      ...
      //} while(sub_g729((*cand1),(NCODE1-NCAN1))) ;
      } while (*cand1 != (NCODE1-NCAN1)) ;
      /*-- pre select codebook #2 --*/
      *cand2 = 0 ;
      do{
      ...
      //} while(sub_g729( (*cand2),(NCODE2-NCAN2))) ;
      } while(*cand2 != (NCODE2-NCAN2)) ;
   }
