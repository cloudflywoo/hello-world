...
)
{
    ...
    Syn_filt_Amr(Aq, ai_zero, h1, L_SUBFR, zero, 0);
    Syn_filt_Amr(Ap2, h1, h1, L_SUBFR, zero, 0);
    ...
    /* LPC residual */
    Residu_Amr(Aq, speech, res2, L_SUBFR);

    oscl_memcpy(exc, res2, L_SUBFR*sizeof(Word16));

    Syn_filt_Amr(Aq, exc, error, L_SUBFR, mem_err, 0);

    Residu_Amr(Ap1, error, xn, L_SUBFR);

    /* target signal xn[]*/
    Syn_filt_Amr(Ap2, xn, xn, L_SUBFR, mem_w0, 0);
