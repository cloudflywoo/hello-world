   ...
   )
{
   ...
   Lsp_Az_Amr(lsp_int, acoeff);
   Lsp_Az_Amr(lsp_int_variab, acoeff_variab);
   ...
   for (i = 0; i < 4; i++)
   {
      ...
      // Synthesize
      Syn_filt_Amr(acoeff_variab, ex, &synth[i * L_SUBFR], L_SUBFR,
               mem_syn, 1);
...
)
{
    ...
    Lsp_Az_Amr(lsp_int, acoeff, pOverflow);
    Lsp_Az_Amr(lsp_int_variab, acoeff_variab, pOverflow);
