...
void Bits2prm(
    ...
    Word16 bits[],      /* i : serial bits       (size <= MAX_SERIAL_SIZE) */
    Word16 prm[],        /* o : analysis parameters  (size <= MAX_PRM_SIZE) */
    CommonAmrTbls* common_amr_tbls /* i : ptr to strcut of table_amr ptrs        */
)
{
    Word16 i;
...
------------------------------------------------------------------------------
 ...
 filtering of the decoded data is done via a call to Post_Filter_Amr function.
 Lastly, the decoded data is post-processed via a call to Post_Process_Amr
...
)
{
  ...
  Post_Filter_Amr(st->post_state, mode, synth, Az_dec);   // Post-filter

  // post HP filter, and 15->16 bits
  Post_Process_Amr(st->postHP_state, synth, L_FRAME);
