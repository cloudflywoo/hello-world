...
------------------------------------------------------------------------------
 ...
 Local Variables Needed:
    table_amr = Log2 table_amr of constants of type Word16

------------------------------------------------------------------------------
 FUNCTION DESCRIPTION

 The function Log2(L_x) calculates the logarithm of the normalized input
 buffer L_x. The logarithm is approximated by a table_amr and linear
 ...
 3. a = bit10-b24
 4. i = i - 32
 5. fraction = table_amr[i]<<16 - (table_amr[i] - table_amr[i+1]) * a * 2
...
)
{
    ...
    *exponent = sub (30, exp);
    ...
    L_y = L_deposit_h (table_amr[i]);       // table_amr[i] << 16
    tmp = sub (table_amr[i], table_amr[i + 1]); // table_amr[i] - table_amr[i+1]
...
)
{
    ...
    else
    {
        ...
        *exponent = 30 - exp;
        ...
        /* Calculate table_amr index -> subtract by 32 is done for           */
        /* proper table_amr indexing, since 32<=i<=63 (due to normalization) */
        i -= 32;

        /* Fraction part of Log2 is approximated by using table_amr[]    */
        /* and linear interpolation, i.e.,                           */
        /* fraction = table_amr[i]<<16 - (table_amr[i] - table_amr[i+1]) * a * 2 */
        L_y = (Word32) log2_tbl[i] << 16;  /* table_amr[i] << 16        */
        tmp = log2_tbl[i] - log2_tbl[i + 1];  /* table_amr[i] - table_amr[i+1] */
