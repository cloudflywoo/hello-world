...
#define NB_PULSE  2
    ...
    static void search_2i40(
        ...
        const Word16* startPos_ptr, /* i: ptr to read only table_amr                 */
    ...
    */
    ...
    Word16 code_2i40_9bits(
        ...
        Word16 y[],         /* o : filtered fixed codebook excitation       */
        Word16 * sign,      /* o : Signs of 2 pulses                        */
        const Word16* startPos_ptr, /* ptr to read-only table_amr              */
        Flag   * pOverflow  /* o : Flag set when overflow occurs            */
    )
    {
        Word16 codvec[NB_PULSE];
    ...
    ------------------------------------------------------------------------------
     ...
     Local Variables Needed:
        startPos = table_amr containing the start positions used by fixed codebook
    ...
    */
    ...
    ------------------------------------------------------------------------------
     ...
     Local Variables Needed:
        trackTable = table_amr used for tracking codewords (Word16)
    ...
    )
    {
        ...
        for (k = 0; k < NB_PULSE; k++) {
           ...
           if (first == 0) {
              ...
              if (k == 0) {
                 track = 0;
                 index = add(index, 64);  // table_amr bit is MSB
              } else {
                 track = 1;
                 index = shl(index, 3);
              }
