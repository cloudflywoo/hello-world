...
------------------------------------------------------------------------------
 ...
 This function initializes the state memory used by the Post_Filter_Amr function
 to zero.
...
)
{
    ...
    for (i_subfr = 0; i_subfr < L_FRAME; i_subfr += L_SUBFR)
    {
       ...
       Residu_Amr (Ap3, &syn_work[i_subfr], st->res2, L_SUBFR);
       ...
       Syn_filt_Amr (Ap4, h, h, L_H, &h[M + 1], 0);
       ...
       preemphasis_amr (st->preemph_state, st->res2, temp2, L_SUBFR);

       // filtering through  1/A(z/0.75)

       Syn_filt_Amr (Ap4, st->res2, &syn[i_subfr], L_SUBFR, st->mem_syn_pst, 1);

       // scale output to input

       agc_amr (st->agc_state, &syn_work[i_subfr], &syn[i_subfr],
            AGC_FAC, L_SUBFR);
...
)
{
    ...
    for (i_subfr = 0; i_subfr < L_FRAME; i_subfr += L_SUBFR)
    {
        ...
        preemphasis_amr(&(st->preemph_state), st->res2, temp2, L_SUBFR, pOverflow);
