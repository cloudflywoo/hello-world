    ...
    *------------------------------------------------------------------------*
    ...
    for (i_subfr = 0; i_subfr < L_FRAME; i_subfr += L_SUBFR)
    {
       ...
       if (sub(mode, MR122) != 0)
       {
          ...
          Dec_lag3_Amr (index, t0_min, t0_max, pit_flag, st->old_T0,
                    &T0, &T0_frac, flag4);
        ...
        *-------------------------------------------------------*
        ...
        if (sub (mode, MR475) == 0)
        {
           ...
           if (bfi == 0)
           {
              Dec_gain_amr(st->pred_state, mode, index_mr475, code,
                 ...
                 (sub (mode, MR102) == 0))
        {
            ...
            if (bfi == 0)
            {
               Dec_gain_amr(st->pred_state, mode, index, code,
                        evenSubfr, &gain_pit, &gain_code);
            }
            else
            {
        ...
        if (sub (pit_sharp, 16384) > 0)
        {
           ...
           Overflow = 0;
           Syn_filt_Amr (Az, excp, &synth[i_subfr], L_SUBFR,
                     st->mem_syn, 0);
        }
        else
        {
           Overflow = 0;
           Syn_filt_Amr (Az, exc_enhanced, &synth[i_subfr], L_SUBFR,
                     st->mem_syn, 0);
        }
        ...
        if (Overflow != 0)    // Test for overflow
        {
           ...
           Syn_filt_Amr(Az, exc_enhanced, &synth[i_subfr], L_SUBFR, st->mem_syn, 1);
        }
        else
        {
