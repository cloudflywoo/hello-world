...
extern "C"
{
    ...
    extern const Word16 table_amr[];
    extern const Word16 slope_amr[];
...
------------------------------------------------------------------------------
 ...
 The transformation from lsp[i] to lsf[i] is approximated by a look-up table_amr
 and interpolation.
...
)
{
    ...
    for (i = 0; i < m; i++)
    {
        ...
        // lsp[i] = table_amr[ind]+ ((table_amr[ind+1]-table_amr[ind])*offset) / 256

        L_tmp = L_mult (sub (table_amr[ind + 1], table_amr[ind]), offset);
        lsp[i] = add (table_amr[ind], extract_l (L_shr (L_tmp, 9)));
...
)
{
    ...
    for (i = 0; i < m; i++)
    {
        ...
        L_tmp = ((Word32)(table_amr[ind + 1] - table_amr[ind]) * offset) >> 8;
        lsp[i] = table_amr[ind] + (Word16) L_tmp;
...
------------------------------------------------------------------------------
 ...
 Global Variables Used:
    table_amr = cosine table_amr
    slope_amr = table_amr to used to calculate inverse cosine
...
------------------------------------------------------------------------------
 ...
 The transformation from lsf[i] to lsp[i] is approximated by a look-up table_amr
 and interpolation.
...
)
{
    ...
    ind = 63;                        // begin at end of table_amr -1

    for (i = m - 1; i >= 0; i--)
    {
        // find value in table_amr that is just greater than lsp[i]

        while (sub (table_amr[ind], lsp[i]) < 0)
        {
            ind--;

        }

        // acos(lsp[i])= ind*256 + ( ( lsp[i]-table_amr[ind] ) *
           slope_amr[ind] )/4096

        L_tmp = L_mult (sub (lsp[i], table_amr[ind]), slope_amr[ind]);
        //(lsp[i]-table_amr[ind])*slope_amr[ind])>>12
...
)
{
    ...
    for (i = m - 1; i >= 0; i--)
    {
        ...
        L_tmp = (Word32)(temp - table_amr[ind]) * slope_amr[ind];
