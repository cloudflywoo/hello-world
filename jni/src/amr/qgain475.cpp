...
/* The table_amr contains the following data:
 *
...
------------------------------------------------------------------------------
 ...
 Inputs:
    ...
    p = pointer to selected quantizer table_amr entry (const Word16)
...
static void MR475_quant_store_results(
    ...
    const Word16 *p,       // i  : pointer to selected quantizer table_amr entry
...
)
{
     ...
     *  ~~~~~~~~~~~~~~~~                                                 *
     *                                                                   *
     *  For each pair (g_pitch, g_fac) in the table_amr calculate the        *
     *  terms t[0..4] and sum them up; the result is the mean squared    *
     *  error for the quantized gains from the table_amr. The index for the  *
    ...
    for (i = 0; i < MR475_VQ_SIZE; i++)
    {
        ...
        if (tmp <= 0 && sub(g_pitch, gp_limit) <= 0)
        {
            ...
            // store table_amr index if MSE for this index is lower
