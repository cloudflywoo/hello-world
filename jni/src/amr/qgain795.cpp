...
MR795_gain_code_quant3(
    ...
    /*      (for other MA predictor update)   */
    const Word16* qua_gain_code_ptr, /* i : ptr to read-only table_amr      */
...
)
{
     ...
     *  t[0..4] are calculated from the values in the table_amr (and the     *
...
)
{
    ...
    /* search the quantizer table_amr for the lowest value
    ...
    for (i = 0; i < NB_QUA_CODE; i++)
    {
        ...
        /* store table_amr index if distance measure for this
...
MR795_gain_quant(
    ...
    /*      (first gain pitch, then code pitch)*/
    CommonAmrTbls* common_amr_tbls, /* i : ptr to struct of table_amr ptrs   */
    Flag   *pOverflow         /* o  : overflow indicator                */
)
{
    Word16 frac_en[4];
