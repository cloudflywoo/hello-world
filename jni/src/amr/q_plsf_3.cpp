...
------------------------------------------------------------------------------
 ...
 Global Variables Used:
    pred_fac = table_amr containing prediction factors (const Word16)
    dico1_lsf = quantization table_amr for split_MQ of 2 sets of LSFs
                in a 20 ms frame (const Word16)
    dico2_lsf = quantization table_amr for split_MQ of 2 sets of LSFs
                in a 20 ms frame (const Word16)
    dico3_lsf = quantization table_amr for split_MQ of 2 sets of LSFs
                in a 20 ms frame (const Word16)
    mr515_3_lsf = third codebook for MR475 and MR515 modes (const Word16)
    mr795_1_lsf = first codebook for MR795 mode (const Word16)
    mean_lsf = table_amr of mean LSFs (const Word16)
    past_rq_init = initalization table_amr for MA predictor in DTX mode
                   (const Word16)
