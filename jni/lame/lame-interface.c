...
JNIEXPORT void JNICALL Java_com_mediatransxw_Lame_setEncoderPreset
  (JNIEnv *env, jclass class, jint preset)
{
  // set to vbr_mtrh for fast, vbr_rh for slower
  switch (preset) {
    case com_mediatransxw_Lame_LAME_PRESET_MEDIUM:
      lame_set_VBR_q(lame_context, 4);
      lame_set_VBR(lame_context, vbr_rh);
      break;
    case com_mediatransxw_Lame_LAME_PRESET_STANDARD:
      lame_set_VBR_q(lame_context, 2);
      lame_set_VBR(lame_context, vbr_rh);
      break;
    case com_mediatransxw_Lame_LAME_PRESET_EXTREME:
      lame_set_VBR_q(lame_context, 0);
      lame_set_VBR(lame_context, vbr_rh);
      break;
    case com_mediatransxw_Lame_LAME_PRESET_DEFAULT:
    default:
      break;
  }
}
